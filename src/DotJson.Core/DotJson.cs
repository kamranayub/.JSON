...
namespace DotJson {
    ...
    public class Json : DynamicObject {
        ...
        private Json(IDictionary<string, object> dictionary) {
            // Convert to concrete dictionary to avoid { { "Key": "someting", "Value": "value" } }
            // situations for derived dictionaries.
            this.OriginalObject = new Dictionary<string, object>(dictionary);
        ...
        /// <param name="pretty">Whether or not to pretty format the output</param>
        /// <returns>A JSON String</returns>
        public static string Stringify(IDictionary<string, object> dict, bool pretty = false) {
            return new Json(dict).ToString(pretty);
        }
